...
module.exports = async(img, title, subtitle) => {
  if(title.length >= 24) {
    const canvas = createCanvas(712, 512);
    const ctx = canvas.getContext('2d');
    
    const asset = await loadImage('../assets/big_demotivator.png');
    ctx.drawImage(asset, 0, 0);
  
    const avatar = await loadImage(img);
    ctx.drawImage(avatar, 133, 33, 446, 390);
    ...
    let widthText = 470
    if(subtitle) ctx.fillText(title, 360, widthText - 10);
    if(!subtitle) ctx.fillText(title, 360, widthText);
  
    if(subtitle) {
    ctx.font = `20px Times New Roman`;
    ctx.fillText(subtitle, 360, widthText + 20);
    };
